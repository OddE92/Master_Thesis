...
int Guiding_Center::initialize_new_GC(Particle &particle, Bfield &bfield, Ran &rng, Seconds t){
 ...
 // Calculate initial GC-velocity
  particle.v_parallell = calculate_v_parallell(particle.v, bfield.B_hat);          // v_parallell = v * \hat{b}

      // GC.v = \vect{v_parallell} = v_parallell * \hat{b} at start point
  GC_velocity = { particle.v_parallell*bfield.B_hat[0], particle.v_parallell*bfield.B_hat[1], particle.v_parallell*bfield.B_hat[2] };
 ...
 // Calculate initial GC-position
  particle.v_perp = calculate_v_perp(particle.v, particle.v_parallell);
  R_Larmor = 1.0810076e-15 * (particle.v_perp / GCT::c) * (particle.E / GCT::vector_amplitude(bfield.B));
  ...
  GC_position = { particle.pos[0] + R_Larmor*a_hat[0], 
                  particle.pos[1] + R_Larmor*a_hat[1], 
                  particle.pos[2] + R_Larmor*a_hat[2] 
                };

  return 0;
}

double Guiding_Center::calculate_v_parallell(const std::array<mps, 3> &particle_velocity, const std::array<double, 3> &B_hat){
  return GCT::vector_dot_product(particle_velocity, B_hat);
}

double Guiding_Center::calculate_v_perp(const std::array<mps, 3> &particle_velocity, mps v_parallell){
