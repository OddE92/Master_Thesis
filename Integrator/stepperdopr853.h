#ifndef STEPPERDOPR853
#define STEPPERDOPR853
...
StepperDopr853<D>::StepperDopr853(VecDoub_IO &yy, VecDoub_IO &dydxx, Doub &xx,
                                  const Doub atoll, const Doub rtoll, bool dens) : StepperBase(yy, dydxx, xx, atoll, rtoll, dens), yerr2(n), k2(n), k3(n), k4(n),
...
void StepperDopr853<D>::step(const Doub htry, D &derivs)
{
    ...
    for (int i = 0; i < 100; i++)
    {
        ...
        if (con.success(err,h)){
            if(i > 25) std::cout << "Ok, moving on. \n"; 
            break;
        }

        if (abs(h) <= abs(x) * EPS){
            std::cout << "stepsize underlow in StepperDopr853 \n";
...
Doub StepperDopr853<D>::error(const Doub h)
{
    ...
    for (Int i = 0; i < n; i++)
    {
        ...
        //std::cout << "max: " << MAX(abs(y[i]), abs(yout[i])) << endl;
        err2 += SQR(yerr[i] / sk);
        err += SQR(yerr2[i] / sk);
        //std::cout << "sk: " << sk << endl;
        //std::cout << "yerr[i]: " << yerr[i] << endl;
        //std::cout << "yerr2[i] " << yerr2[i] << endl;
