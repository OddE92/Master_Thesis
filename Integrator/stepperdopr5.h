...
bool StepperDopr5<D>::Controller::success(const Doub err,Doub &h) {
	...
	if (err <= 1.0) {
		...
		/*std::cout << "Rejecting error: " err << endl;
		std::cout << safe*pow(err, -alpha) << endl;
		std::cout << scale;*/
		h *= scale;
		reject=true;
		//std::cout << endl << "Step was rejected. Next h: " << h <<  endl;
		return false;
	}
}
template <class D>
...
Doub StepperDopr5<D>::error() {
	...
	/*	std::cout << "sk: " << sk << endl;
		std::cout << "yerr: " << yerr[i] << endl;
		std::cout << "yerr/sk: " << yerr[i]/sk << endl;
		std::cout << "square: " << SQR(yerr[i]/sk) << endl << endl;
	*/	err += SQR(yerr[i]/sk);
	}
	//std::cout << "error, n:" << n << endl;
	//std::cout << "error, err: " << err << endl;
	return sqrt(err/n);
}
template <class D>
